...
![MCAR, MAR, and MNAR Example, where M is the probability of a missing data point, X is the observed data, and Y is the unobserved data. [@du2020missing]](Missing Data Mechanisms Figure.png)


Understanding the mechanism behind missing data (MCAR, MAR, or NMAR) is important because many imputation methods require the assumption of MCAR or MAR to be held true. The Little's MCAR test is a statistical test that can be used to support whether data are MCAR or not. This method will be discussed further in the methods section. Visualization of the missing data is helpful for determining the relationships (or lack thereof) within the missing data. For example, correlation matrix plots, scatterplots, and other custom visualizations can be useful tools in identifying patterns in the missing data [@ghazali2020missing]. 


#### **Techniques for Handling Missing Data**(RS)
...
Data were classified as either MCAR or not MCAR using the Little's test of missing completely at random [@li2013little]. This is a chi-squared test that evaluates whether mean differences exists between groups with different missing data patterns. In this test, the hypotheses are as follows:
...
Note, this procedure is only designed to test between MCAR/not MCAR and is not able to classify data as MAR or MNAR. Data visualization is recommended to provide supportive information to support MAR assumptions.
...
![Multiple Imputation versus Single Imputation Methods, [@ren2023review]](Multiple Imputation vs Single Imputation.png)
...
![Visual representation of decision tree algorithm (https://365datascience.com/tutorials/machine-learning-tutorials/decision-trees/)](Decision Tree Example.png)
...
![Visual representation of random forest algorithm (https://medium.com/@denizgunay/random-forest-af5bde5d7e1e)](Random Forest Example.png)
...
![Visual representation of KNN algorithm, where k is the number of nearest neighbors (https://towardsdatascience.com/why-does-increasing-k-decrease-variance-in-knn-9ed6de2f5061)](KNN Example.png)
...
This work demonstrates various methodologies that can be used to handle missing data. The dataset "Ozone", from the mlbench package in R was used to demonstrate the techniques for either deleting or imputing missing data. Visualization techniques to help classify data as MCAR, MAR, and NMAR were also demonstrated. With respect to the "Ozone" dataset, deletion of the column Temp_EM, which contains ~38% missing data, was found to be the most effective method for handling missing data. This was demonstrated through machine learning models trained to datasets where missing data were handled using various techniques including listwise deletion, feature selection (column deletion), mean/median/mode imputation, random forest (missForest) and MICE. The random forest model in combination with feature selection for handling missing data resulted in the lowest RMSE with respect to the test (unseen) dataset.
